...
def framing(frame_len_s, frame_shift_s, fs, sig):
    ...
    """
    sig_n = len(sig)
    frame_len_n, frame_shift_n = int(round(fs * frame_len_s)), int(
        round(fs * frame_shift_s))
...
def mel_filter(frame_pow, fs, n_filter, nfft):
    ...
    mel_points = np.linspace(mel_min, mel_max,
                             n_filter + 2)  # n_filter个mel值均匀分布与最低与最高mel值之间
